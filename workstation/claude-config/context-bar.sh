#!/bin/bash
# =============================================================================
# context-bar.sh — Shared status line for multi-profile Claude Code
#
# Each Claude profile should have a `.claude-profile` file containing the
# profile name (e.g., "personal", "work", "alt") that matches an entry in
# ~/.config/claude-profiles/profiles.json
#
# Settings reference in settings.json:
#   "statusLine": { "type": "command", "command": "~/.config/claude-profiles/context-bar.sh" }
# =============================================================================

PROFILES_DIR="$HOME/.config/claude-profiles"
PROFILES_JSON="$PROFILES_DIR/profiles.json"
PROFILE_FILE="$HOME/.claude-profile"
...
# Read profile name from config
if [[ -f "$PROFILE_FILE" ]]; then
    read -r PROFILE_NAME < "$PROFILE_FILE"
fi

# Look up profile color/label from profiles.json
if [[ -f "$PROFILES_JSON" ]]; then
    if command -v jq &>/dev/null; then
        COLOR=$(jq -r --arg p "$PROFILE_NAME" '.[$p].color // "blue"' "$PROFILES_JSON")
        LABEL=$(jq -r --arg p "$PROFILE_NAME" '.[$p].label // "CLAUDE"' "$PROFILES_JSON")
        EMOJI=$(jq -r --arg p "$PROFILE_NAME" '.[$p].emoji // ""' "$PROFILES_JSON")
    fi
fi

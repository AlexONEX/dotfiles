...
local servers = {
  ...
  { "jsonls",          "vscode-json-languageserver",    "npm i -g vscode-langservers-extracted" },
...
local extras = {
  { "pylsp-rope",    "python3 -c 'import pylsp_rope'",  "pip install pylsp-rope" },
  { "rope",          "python3 -c 'import rope'",         "pip install rope" },
...
function M.check()
  ...
  for _, entry in ipairs(extras) do
    local name, check_cmd, hint = entry[1], entry[2], entry[3]
